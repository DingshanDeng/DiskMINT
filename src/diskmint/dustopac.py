...
# created: 04/14/2025
# new version updated: 04/14/2025
# DiskMINT v1.5.0


"""
Many of the following functions 
are based on the radmc3dPy v2.0.0 
by Attila Juhasz, 
maintained by Cornelis Dullemond

manully took out the radmc3dOctree support below
"""
...
try:
    ...
    scipy = None
    sp = None
    print(' Numpy or Scipy or subprocess cannot be imported ')
    print(' To use this python module, you need to install Numpy, Scipy, and subprocess')
    ...
    def readOpac(self, ext=None, idust=None, scatmat=None, old=False):
        ...
        for i in range(len(ext)):
            if scatmat[i]:
                ...
                self.freq.append(const.cc / data_opac[:, 0] * 1e4)
                ...
                if not old:
                    ...
                    if iformat == 1:
                        ...
                        self.freq.append(const.cc / data[:, 0] * 1e4)
        ...
        if len(ppar['lnk_fname']) > 1:
            ...
            for idust in range(len(ppar['lnk_fname'])):
                ...
                if code.lower().strip() == 'fortran':
                    # Run makedust
                    self.runMakedust(freq=const.cc / wav * 1e4, gmin=ppar['gsmin'], gmax=ppar['gsmax'], ngs=ppar['ngs'],
