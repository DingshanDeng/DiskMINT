...
Run the main script from inside the `example_RULup/` directory. (It is recommended to run it inside a `screen` session so it can continue if your connection drops.)
