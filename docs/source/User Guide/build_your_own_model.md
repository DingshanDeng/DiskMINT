...
2. Copy all `dustkappa_optool_20250122_*.inp` files, plus `aave.inp`, `fracs.inp`, `fracs_numb.inp`, and `ndsd.inp`, into your `data_dir`.
...
Then set `bool_MakeDustKappa = True` in your run script to let DiskMINT compute `aave.inp`, `ndsd.inp`, `fracs.inp`, and `fracs_numb.inp` automatically from `amin_1`, `amax_1`, `pla_dustsize`, and `rhobulk`.
...
mint.bool_VHSE          = True
mint.n_vhse_loop        = 10   # number of RT+hydrostatic iterations
...
### C18O reliability note

The chemical network (Ruaud et al. 2022) is most reliable for C18O emission from warm molecular layers at intermediate heights. CO ice in the cold midplane and UV-driven photodissociation near the surface are both included, but the network does not resolve short-timescale transients. If your target has strong UV variability, treat the C18O abundance in the upper disk layers as an order-of-magnitude estimate.
