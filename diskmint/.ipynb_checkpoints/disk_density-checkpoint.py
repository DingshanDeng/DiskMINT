...
def place_ratio_g2d(ratio_g2d, shape, x_fit):
    ...
    """
    if type(ratio_g2d) in [np.int, np.float]:
        print('the g2d ratio set up is a constant (float or int), with ratio_g2d=%.2f'%(ratio_g2d))
...
def problem_setup(model):
    ...
    if para.fmodel_filename != '':
        with open(os.path.join(model.file_dir,'stars.inp'),'w+') as f:
            f.write('2\n')
            f.write('1 %d\n\n'%(model.nlam))
            f.write('%13.13e %13.13e %13.13e %13.13e %13.13e\n\n'%(para.rstar,para.mstar,para.pstar[0],para.pstar[1],para.pstar[2]))
            for value in model.lam:
                f.write('%13.6e\n'%(value))
        ...
        with open(os.path.join(model.file_dir,'stars.inp'),'w+') as f:
            f.write('2\n')
            f.write('1 %d\n\n'%(model.nlam))
            f.write('%13.13e %13.13e %13.13e %13.13e %13.13e\n\n'%(model.rstar,model.mstar,model.pstar[0],model.pstar[1],model.pstar[2]))
            for value in model.lam:
                f.write('%13.6e\n'%(value))
            f.write('\n%13.6e\n'%(-para.tstar))
...
def write_chem_output(model, chem_code_dir, chem_code_name='reduced_withCO2'):
    """
    ...
    print('Done, saving file to %s'%(write_dir+'COendgrid-'+ chem_code_name +'-'+ save_name + '-cyl.chem'))
